...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
    ...
    movieTabNavigationController = UINavigationController.init(rootViewController: MoviesViewController())
    movieTabNavigationController.tabBarItem = UITabBarItem(title: "Movies", image: UIImage(named: "list_icon"), tag: 0)
    
    favoriteTabNavigationControoller = UINavigationController.init(rootViewController: FavoritesViewController())
