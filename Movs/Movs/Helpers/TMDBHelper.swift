...
fileprivate let GENRE_MOVIE_PATH = "\(ENDPOINT)genre/movie/list"
...
fileprivate let JSON_GENRES_KEY = "genres"
...
class TMDBHelper {
    ...
    func getListOfGenres(_ block: @escaping (_ error: Error?, _ genreList: [GenreModel]?)->Void){
        let url = GENRE_MOVIE_PATH
        let param = [P_API_KEY: API_KEY]
        ...
        Alamofire.request(url, method: .get, parameters: param).responseJSON { (response) in
            ...
            case .success:
                if let value = response.result.value{
                    let json = JSON(value)
                    let genreArray = json[JSON_GENRES_KEY].arrayValue.map({GenreModel(json: $0)})
                    block(nil, genreArray)
                }else{ block(nil, nil) }
            }
        }
    }
